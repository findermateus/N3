...
namespace Anotacoes
{
    public class EditoraCRUD
    {
        ...
        public void executarCRUD()
        {
            ...
            {
                // cadastro
                resp = tl.fazerPergunta(11, 9, "Registro NÃO encontrado. Deseja cadastrar (S/N):");
                if (resp.ToUpper() =="S")
                {
                    this.entrarDados();
                    resp = tl.fazerPergunta(11, 9, "Confirma cadastro (S/N):");
                    if (resp.ToUpper() =="S")
                    {
            ...
            else
            {
                ...
                this.mostrarDados();
                resp = tl.fazerPergunta(11, 9, "Deseja alterar/excluir/voltar (A/E/V):");
                if (resp.ToUpper() == "A")
                {
                    this.tl.limparArea(20, 8, 69, 8);
                    this.entrarDados();
                    resp = tl.fazerPergunta(11, 9, "Confirma alteração (S/N):");
                    if (resp.ToUpper() == "S")
                    {
                ...
                if (resp.ToUpper() == "E")
                {
                    resp = tl.fazerPergunta(11, 9, "Confirma exclusão (S/N):");
                    if (resp.ToUpper()=="S")
                    {
        ...
        {
            tl.montarMoldura(10, 5, 70, 10, "Cadastro de Editoras");
